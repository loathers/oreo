...
function isSong(skillOrEffect) {
  if (skillOrEffect instanceof kolmafia.Effect && skillOrEffect.attributes.includes("song")) {
    ...
    return skill.class === $class(_templateObject$8 || (_templateObject$8 = _taggedTemplateLiteral(["Accordion Thief"]))) && skill.buff;
  }
}
/**
...
function damageTakenByElement(baseDamage, element) {
  var res = kolmafia.elementalResistance(element);
...
function extractItems(text) {
  return new Map(Object.entries(kolmafia.extractItems(text)).map(_ref13 => {
    var _ref14 = _slicedToArray(_ref13, 2),
      itemName = _ref14[0],
      quantity = _ref14[1];
    return [kolmafia.Item.get(itemName), quantity];
  }));
}
...
var cachedSlots = $slots(_templateObject8$4 || (_templateObject8$4 = _taggedTemplateLiteral(["hat, weapon, off-hand, back, shirt, pants, acc1, acc2, acc3, familiar"])));
...
function verifyCached(entry) {
  ...
  if (kolmafia.equippedAmount($item(_templateObject28$2 || (_templateObject28$2 = _taggedTemplateLiteral(["Buddy Bjorn"])))) > 0 && entry.rider.get($item(_templateObject29$2 || (_templateObject29$2 = _taggedTemplateLiteral(["Buddy Bjorn"]))))) {
    if (entry.rider.get($item(_templateObject30$2 || (_templateObject30$2 = _taggedTemplateLiteral(["Buddy Bjorn"])))) !== kolmafia.myBjornedFamiliar()) {
      if (warn) {
        logger.warning("Failed to apply".concat(entry.rider.get($item(_templateObject31$2 || (_templateObject31$2 = _taggedTemplateLiteral(["Buddy Bjorn"])))), " in ").concat($item(_templateObject32$1 || (_templateObject32$1 = _taggedTemplateLiteral(["Buddy Bjorn"]))), "."));
      }
      success = false;
    }
  }
...
function maximizeCached(objectives) {
  ...
  var cacheKey = [objective].concat(_toConsumableArray(untouchedSlots.map(slot => "".concat(slot, ":").concat(kolmafia.equippedItem(slot))).sort()), [have($effect(_templateObject48$1 || (_templateObject48$1 = _taggedTemplateLiteral(["Offhand Remarkable"])))), options.updateOnLocationChange && kolmafia.myLocation()]).join("; ");
...
(function (Mine) {
  ...
  Mine[Mine["ITZNOTYERZITZ"] = 1] = "ITZNOTYERZITZ";
  /** Deep Inside the Knob Shaft */
  Mine[Mine["KNOB"] = 2] = "KNOB";
  /** Anemone Mine */
  Mine[Mine["ANEMONE"] = 3] = "ANEMONE";
  /** The Gummi Mine (Retired, Crimbo 2011) */
  Mine[Mine["GUMMI"] = 4] = "GUMMI";
  /** Crimbonium Mine (Retired, Crimbo 2014) */
  Mine[Mine["CRIMBONIUM"] = 5] = "CRIMBONIUM";
  /** The Velvet / Gold Mine */
  Mine[Mine["VOLCANO"] = 6] = "VOLCANO";
})(Mine || (Mine = {}));
/**
...
function hasObjectDetection() {
  ...
  if (mine === Mine.CRIMBONIUM && have($effect(_templateObject$6 || (_templateObject$6 = _taggedTemplateLiteral(["Crimbonar"]))))) return true;
  return kolmafia.haveEffect($effect(_templateObject2$5 || (_templateObject2$5 = _taggedTemplateLiteral(["Object Detection"])))) !== 0 || kolmafia.isWearingOutfit("Dwarvish War Uniform");
}
/**
 * @param mine Which mine
...
function caveInCost(mine) {
  switch (mine) {
    ...
    case Mine.ANEMONE:
      return kolmafia.myBuffedstat($stat(_templateObject5$4 || (_templateObject5$4 = _taggedTemplateLiteral(["muscle"])))) * 2.5;
    case Mine.VOLCANO:
      return damageTakenByElement(75, $element(_templateObject6$4 || (_templateObject6$4 = _taggedTemplateLiteral(["hot"]))));
    default:
      return 0;
  }
}
 ...
 */
function mineCoordinate$1(mine, _ref3) {
  var _ref4 = _slicedToArray(_ref3, 2),
    col = _ref4[0],
    row = _ref4[1];
  var page = kolmafia.visitUrl("mining.php?mine=".concat(mine, "&which=").concat(col + 8 * row, "&pwd"), true);
 ...
 */
function visit(mine) {
  return kolmafia.visitUrl("mining.php?mine=".concat(mine));
}
/**
 * @param mine Which mine
...
function countFreeMines() {
  return (have($skill(_templateObject7$4 || (_templateObject7$4 = _taggedTemplateLiteral(["Unaccompanied Miner"])))) ? 5 - get("_unaccompaniedMinerUsed") : 0) + kolmafia.haveEffect($effect(_templateObject8$3 || (_templateObject8$3 = _taggedTemplateLiteral(["Loded"]))));
}
...
function mySessionItemsWrapper() {
  ...
  if (!sessionOnly) {
    for (var _i = 0, _Object$entries = Object.entries(kolmafia.getCampground()); _i < _Object$entries.length; _i++) {
      ...
      if (item === $item(_templateObject31$1 || (_templateObject31$1 = _taggedTemplateLiteral(["big rock"])))) continue; // Used to represent an empty house slot
...
var Outfit = /*#__PURE__*/function () {
  ...
  return _createClass(Outfit, [{
    ...
    value: function equipNonAccessory(item, slot) {
      if ($slots(_templateObject$4 || (_templateObject$4 = _taggedTemplateLiteral(["acc1, acc2, acc3"]))).includes(kolmafia.toSlot(item))) return false;
      ...
      switch (kolmafia.toSlot(item)) {
        ...
        case $slot(_templateObject5$2 || (_templateObject5$2 = _taggedTemplateLiteral(["familiar"]))):
          ...
          break;
        case $slot(_templateObject6$2 || (_templateObject6$2 = _taggedTemplateLiteral(["weapon"]))):
          if (!weaponsCompatible(item, this.equips.get($slot(_templateObject7$2 || (_templateObject7$2 = _taggedTemplateLiteral(["off-hand"])))))) return false;
          break;
      }
    ...
    value: function equipUsingDualWield(item, slot) {
      ...
      if (!kolmafia.canEquip(item)) return false;
      if (!weaponsCompatible(this.equips.get($slot(_templateObject16 || (_templateObject16 = _taggedTemplateLiteral(["weapon"])))), item)) return false;
      this.equips.set($slot(_templateObject17 || (_templateObject17 = _taggedTemplateLiteral(["off-hand"]))), item);
      return true;
    }
  }, {
    ...
    value: function getHoldingFamiliar(item) {
      switch (kolmafia.toSlot(item)) {
        case $slot(_templateObject18 || (_templateObject18 = _taggedTemplateLiteral(["weapon"]))):
          return $familiar(_templateObject19 || (_templateObject19 = _taggedTemplateLiteral(["Disembodied Hand"])));
        case $slot(_templateObject20 || (_templateObject20 = _taggedTemplateLiteral(["off-hand"]))):
          return $familiar(_templateObject21 || (_templateObject21 = _taggedTemplateLiteral(["Left-Hand Man"])));
        case $slot(_templateObject22 || (_templateObject22 = _taggedTemplateLiteral(["pants"]))):
...
function maxSongs() {
  return have($skill(_templateObject$3 || (_templateObject$3 = _taggedTemplateLiteral(["Mariachi Memory"])))) ? 4 : 3;
}
...
var args = Args.create("oreo", "Selectable strategies for the Velvet / Gold Mine", {
  turns: Args.number({
...
var STRATEGIES = ["pjb", "oreo", "ev", "ev-cluster"];
var VISIBILITIES = ["low", "auto", "high"];
var DEFAULT_VALUES = {
  ore: 25,
  gold: 20000,
  crystal: 69,
...
var TARGETS_PER_MINE = 15;
var ROW_ORE_WEIGHT = [0, 0, 0.1868, 0.4698, 0.6209, 0.67];
var CLUSTER_ROW_WEIGHT = [0, 0, 0.46, 0.46, 0.46, 1];
var CENTER = 3;
var rowOf = index => Math.floor(index / 6);
var colOf = index => index % 6;
...
function coordinateToIndex(_ref) {
  var _ref2 = _slicedToArray(_ref, 2),
    column = _ref2[0],
    row = _ref2[1];
...
function statePositionToIndex(position) {
  var gameRow = Math.floor(position / 6);
  return (5 - gameRow) * 6 + position % 6;
}
function isLegal(index, opened) {
  return !opened.has(index) && (rowOf(index) === 0 || neighbors[index].some(neighbor => opened.has(neighbor)));
}
function minimumCostPaths(opened, starts, tileCost) {
  var distances = Array(36).fill(Infinity);
  var parents = Array(36).fill(-1);
  var visited = new Set();
  var _iterator = _createForOfIteratorHelper(_toConsumableArray(starts).sort((a, b) => a - b)),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
  ...
  for (var step = 0; step < 36; step++) {
    ...
    visited.add(current);
    var _iterator2 = _createForOfIteratorHelper(neighbors[current]),
      _step2;
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
        var neighbor = _step2.value;
        if (opened.has(neighbor) || visited.has(neighbor)) continue;
        var candidate = distances[current] + tileCost(neighbor);
        if (candidate < distances[neighbor] || candidate === distances[neighbor] && (parents[neighbor] < 0 || current < parents[neighbor])) {
  ...
  var paths = new Map();
  for (var target = 0; target < 36; target++) {
    if (!Number.isFinite(distances[target])) continue;
    var path = [];
    for (var node = target; node >= 0; node = parents[node]) path.unshift(node);
    paths.set(target, {
...
var ORE_CLUSTERS = (() => {
  var region = Array.from({
    length: 36
  }, (_, index) => index).filter(index => rowOf(index) >= 2);
  var inRegion = new Set(region);
  var seen = new Set();
  var clusters = [];
  var grow = tiles => {
    var key = _toConsumableArray(tiles).sort((a, b) => a - b).join(",");
    if (seen.has(key)) return;
    seen.add(key);
    if (tiles.length === 6) {
      clusters.push(_toConsumableArray(tiles));
      return;
    }
    var present = new Set(tiles);
    var _iterator3 = _createForOfIteratorHelper(tiles),
      _step3;
    try {
      for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
        var tile = _step3.value;
        var _iterator4 = _createForOfIteratorHelper(neighbors[tile]),
          _step4;
        try {
          for (_iterator4.s(); !(_step4 = _iterator4.n()).done;) {
            var neighbor = _step4.value;
            if (inRegion.has(neighbor) && !present.has(neighbor)) grow([].concat(_toConsumableArray(tiles), [neighbor]));
          }
        } catch (err) {
          _iterator4.e(err);
        } finally {
  ...
  var _iterator5 = _createForOfIteratorHelper(region),
    _step5;
  try {
    for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
...
function makeCalibrationBoards(count, seed, secondGoldChance) {
  var random = mulberry32(seed);
  var clusterWeight = ORE_CLUSTERS.reduce((sum, cluster) => sum + cluster.weight, 0);
  var boards = [];
  var _loop = function _loop() {
    var pick = random() * clusterWeight;
    var cluster = ORE_CLUSTERS[ORE_CLUSTERS.length - 1];
    var _iterator6 = _createForOfIteratorHelper(ORE_CLUSTERS),
      _step6;
    try {
      for (_iterator6.s(); !(_step6 = _iterator6.n()).done;) {
    ...
    var ore = new Set(cluster.tiles);
    var goldCount = random() < secondGoldChance ? 2 : 1;
    var caveCount = 6 - goldCount;
    var available = Array.from({
      length: 36
    }, (_, index) => index).filter(index => !ore.has(index));
    for (var index = 0; index < 9; index++) {
      var selected = index + Math.floor(random() * (available.length - index));
      var _ref3 = [available[selected], available[index]];
      available[index] = _ref3[0];
      available[selected] = _ref3[1];
    }
    var board = Array(36).fill("e");
    var _iterator7 = _createForOfIteratorHelper(ore),
      _step7;
    try {
      for (_iterator7.s(); !(_step7 = _iterator7.n()).done;) {
        var _index = _step7.value;
        board[_index] = "o";
      }
    } catch (err) {
      _iterator7.e(err);
    ...
    var _iterator8 = _createForOfIteratorHelper(available.slice(0, goldCount)),
      _step8;
    try {
      for (_iterator8.s(); !(_step8 = _iterator8.n()).done;) {
        var _index2 = _step8.value;
        board[_index2] = "g";
      }
    } catch (err) {
      _iterator8.e(err);
    ...
    var _iterator9 = _createForOfIteratorHelper(available.slice(goldCount, goldCount + 3)),
      _step9;
    try {
      for (_iterator9.s(); !(_step9 = _iterator9.n()).done;) {
    ...
    var _iterator0 = _createForOfIteratorHelper(available.slice(goldCount + 3, goldCount + 3 + caveCount)),
      _step0;
    try {
      for (_iterator0.s(); !(_step0 = _iterator0.n()).done;) {
        var _index4 = _step0.value;
        board[_index4] = "c";
      }
    } catch (err) {
      _iterator0.e(err);
  ...
  for (var boardIndex = 0; boardIndex < count; boardIndex++) {
    _loop();
  }
  return boards;
}
function clusterOrePosterior(knownOre, knownNonOre, sparkleSubset) {
  var mass = Array(36).fill(0);
  var total = 0;
  var _iterator1 = _createForOfIteratorHelper(ORE_CLUSTERS),
    _step1;
  try {
    var _loop2 = function _loop2() {
        var cluster = _step1.value;
        if (_toConsumableArray(knownOre).some(tile => !cluster.tiles.includes(tile))) return 0; // continue
        if (cluster.tiles.some(tile => knownNonOre.has(tile))) return 0; // continue
        if (sparkleSubset && cluster.tiles.some(tile => !sparkleSubset.has(tile))) return 0; // continue
        total += cluster.weight;
        var _iterator10 = _createForOfIteratorHelper(cluster.tiles),
          _step10;
        try {
          for (_iterator10.s(); !(_step10 = _iterator10.n()).done;) {
            var tile = _step10.value;
            mass[tile] += cluster.weight;
          }
        } catch (err) {
          _iterator10.e(err);
        } finally {
  ...
  for (var index = 0; index < mass.length; index++) mass[index] /= total;
  return mass;
}
var StrategyController = /*#__PURE__*/function () {
  function StrategyController(strategy, visibility) {
    var lambdaOverride = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
    var values = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : DEFAULT_VALUES;
    var secondGoldChance = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 0.496;
    var onWarning = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : () => {};
    _classCallCheck(this, StrategyController);
    _defineProperty(this, "knownSparkles", new Set());
    _defineProperty(this, "knownDull", new Set());
    _defineProperty(this, "observed", new Map());
    _defineProperty(this, "opened", new Set());
    _defineProperty(this, "plannedPath", []);
    _defineProperty(this, "fullMineSeen", false);
    _defineProperty(this, "dynamitePrice", Infinity);
    _defineProperty(this, "dynamiteAvailable", 0);
    _defineProperty(this, "stateAvailable", true);
    _defineProperty(this, "warnedImpossibleCluster", false);
  ...
  return _createClass(StrategyController, [{
    key: "reset",
    value: function reset() {
    ...
    key: "update",
    value: function update(rawState, hasObjectDetection) {
      var previouslyMined = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : [];
      ...
      this.stateAvailable = true;
      this.opened = new Set();
      for (var position = 0; position < rawState.length; position++) {
        if (rawState[position] === "o") this.opened.add(statePositionToIndex(position));
      }
      var _iterator11 = _createForOfIteratorHelper(previouslyMined),
        _step11;
      try {
        for (_iterator11.s(); !(_step11 = _iterator11.n()).done;) {
          var _step11$value = _slicedToArray(_step11.value, 2),
            coordinate = _step11$value[0],
            resource = _step11$value[1];
          var _index5 = coordinateToIndex(coordinate);
          if (this.opened.has(_index5)) this.observed.set(_index5, resource);
        }
      } catch (err) {
        _iterator11.e(err);
      } finally {
      ...
      var fullVisibility = this.visibility !== "low" && hasObjectDetection;
      if (fullVisibility) this.fullMineSeen = true;
      for (var _position = 0; _position < rawState.length; _position++) {
        var index = statePositionToIndex(_position);
        if (this.opened.has(index) || !fullVisibility && !isLegal(index, this.opened)) continue;
        if (rawState[_position] === "*") {
      ...
      var _iterator12 = _createForOfIteratorHelper(this.opened),
        _step12;
      try {
        for (_iterator12.s(); !(_step12 = _iterator12.n()).done;) {
          var _index6 = _step12.value;
          this.knownSparkles.delete(_index6);
        }
      } catch (err) {
        _iterator12.e(err);
      } finally {
    ...
    key: "recordMine",
    value: function recordMine(coordinate, resource) {
      var index = coordinateToIndex(coordinate);
      var wasSparkle = this.knownSparkles.has(index);
      this.knownSparkles.delete(index);
      this.opened.add(index);
      if (resource) this.observed.set(index, resource);else if (wasSparkle) this.observed.set(index, "cave");
    }
  }, {
    key: "shouldResetAfterGold",
    value: function shouldResetAfterGold() {
      return this.strategy === "pjb" || this.strategy === "oreo";
    }
  }, {
    key: "needsObjectDetection",
    value: function needsObjectDetection() {
      return this.visibility === "high" && !this.fullMineSeen;
    }
  }, {
    key: "setDynamitePrice",
    value: function setDynamitePrice(price) {
      if (!Number.isFinite(price) || price < 0) throw new Error("Dynamite price must be non-negative.");
      this.dynamitePrice = price;
    }
  }, {
    key: "setDynamiteAvailable",
    value: function setDynamiteAvailable(quantity) {
    ...
    key: "shouldUseDynamite",
    value: function shouldUseDynamite() {
    ...
    key: "communityDecision",
    value: function communityDecision() {
      var candidates = _toConsumableArray(this.knownSparkles).filter(index => rowOf(index) <= 1 && isLegal(index, this.opened)).sort((a, b) => a - b);
      ...
      if (this.opened.size > 0) {
        return {
          action: "reset",
          reason: "no accessible front-two-row sparkle remains"
        };
      }
      var column = CENTER;
      ...
      return {
        ...
        reason: column === CENTER ? "probing the center of the front row" : "probing above the longest second-row sparkle vein"
      };
    }
  }, {
    key: "evDecision",
    value: function evDecision(cluster) {
      var ev = this.expectedValues(cluster);
      var starts = new Set();
      for (var index = 0; index < 6; index++) {
        if (!this.opened.has(index)) starts.add(index);
      }
      var _iterator13 = _createForOfIteratorHelper(this.opened),
        _step13;
      try {
        for (_iterator13.s(); !(_step13 = _iterator13.n()).done;) {
          var opened = _step13.value;
          var _iterator15 = _createForOfIteratorHelper(neighbors[opened]),
            _step15;
          try {
      ...
      var best = null;
      var lambda = this.turnValue();
      var paths = minimumCostPaths(this.opened, starts, index => this.knownDull.has(index) && this.shouldUseDynamite() && this.dynamiteAvailable > 0 ? this.dynamitePrice : lambda);
      var _iterator14 = _createForOfIteratorHelper(_toConsumableArray(this.knownSparkles).sort((a, b) => a - b)),
        _step14;
      try {
        for (_iterator14.s(); !(_step14 = _iterator14.n()).done;) {
          var target = _step14.value;
          var route = paths.get(target);
          if (!route) continue;
          var reward = ev[target];
          var score = reward - route.cost;
      ...
      if (!best) {
        return this.opened.size === 0 ? {
          ...
          action: "reset",
          reason: "no visible sparkle remains"
        };
      }
      if (this.opened.size > 0 && best.score < 0) {
        return {
          action: "reset",
          reason: "best route EV ".concat(best.reward.toFixed(0), " is below its turn/dynamite cost")
        };
      }
      ...
      return {
        ...
        reason: "best route has EV ".concat(best.reward.toFixed(0), ", ").concat(best.path.length, " step(s), \u03BB=").concat(lambda)
      };
    }
  }, {
    key: "turnValue",
    value: function turnValue() {
    ...
    key: "expectedValues",
    value: function expectedValues(cluster) {
      return cluster ? this.clusterExpectedValues() : this.perTileExpectedValues();
    }
  }, {
    key: "remainingCounts",
    value: function remainingCounts(targetCount) {
      var ore = 0;
      var crystal = 0;
      var gold = 0;
      var _iterator16 = _createForOfIteratorHelper(this.observed.values()),
        _step16;
      try {
        for (_iterator16.s(); !(_step16 = _iterator16.n()).done;) {
          var type = _step16.value;
          if (type === "ore") ore++;else if (type === "crystal") crystal++;else if (type === "gold") gold++;
        }
      } catch (err) {
        _iterator16.e(err);
      } finally {
      ...
      var oreRemaining = Math.max(0, 6 - ore);
      var crystalRemaining = Math.max(0, 3 - crystal);
      var goldRemaining = Math.max(0, 1 + this.secondGoldChance - gold);
      return {
        oreRemaining,
        crystalRemaining,
        goldRemaining,
    ...
    key: "perTileExpectedValues",
    value: function perTileExpectedValues() {
      var result = Array(36).fill(0);
      var targets = _toConsumableArray(this.knownSparkles);
      var targetCount = this.fullMineSeen ? targets.length : TARGETS_PER_MINE - this.observed.size;
      var counts = this.remainingCounts(targetCount);
      var oreProbability = new Map();
      if (this.fullMineSeen) {
        var eligible = targets.filter(index => rowOf(index) >= 2 && neighbors[index].some(neighbor => this.observed.get(neighbor) === "ore" || this.knownSparkles.has(neighbor)));
        var weightTotal = eligible.reduce((sum, index) => sum + ROW_ORE_WEIGHT[rowOf(index)], 0);
        var _iterator17 = _createForOfIteratorHelper(eligible),
          _step17;
        try {
          for (_iterator17.s(); !(_step17 = _iterator17.n()).done;) {
            var index = _step17.value;
            oreProbability.set(index, weightTotal > 0 ? Math.min(1, counts.oreRemaining * ROW_ORE_WEIGHT[rowOf(index)] / weightTotal) : 0);
          }
        } catch (err) {
          _iterator17.e(err);
        } finally {
      ...
      try {
        for (_iterator18.s(); !(_step18 = _iterator18.n()).done;) {
          var _index7 = _step18.value;
          var pOre = oreProbability.get(_index7) ?? 0;
          if (!this.fullMineSeen && rowOf(_index7) >= 2 && counts.oreRemaining > 0) {
            var adjacentPossibleOre = neighbors[_index7].some(neighbor => this.observed.get(neighbor) === "ore" || this.knownSparkles.has(neighbor));
            pOre = Math.min(1, ROW_ORE_WEIGHT[rowOf(_index7)] * (adjacentPossibleOre ? 1 : 0.3));
          }
          result[_index7] = this.nonOreAdjustedValue(pOre, counts);
        }
      } catch (err) {
        _iterator18.e(err);
      } finally {
    ...
    key: "clusterExpectedValues",
    value: function clusterExpectedValues() {
      var knownOre = new Set();
      var knownNonOre = new Set();
      var _iterator19 = _createForOfIteratorHelper(this.observed),
        _step19;
      try {
        for (_iterator19.s(); !(_step19 = _iterator19.n()).done;) {
          ...
          if (type === "ore") knownOre.add(index);else knownNonOre.add(index);
        }
      } catch (err) {
        _iterator19.e(err);
      } finally {
      ...
      var _iterator20 = _createForOfIteratorHelper(this.knownDull),
        _step20;
      try {
        for (_iterator20.s(); !(_step20 = _iterator20.n()).done;) {
          var _index8 = _step20.value;
          knownNonOre.add(_index8);
        }
      } catch (err) {
        _iterator20.e(err);
      } finally {
      ...
      var unknownOpened = _toConsumableArray(this.opened).filter(index => !this.observed.has(index) && !this.knownDull.has(index));
      var subset = this.fullMineSeen ? new Set([].concat(_toConsumableArray(this.knownSparkles), _toConsumableArray(knownOre), _toConsumableArray(unknownOpened))) : null;
      var pOre = clusterOrePosterior(knownOre, knownNonOre, subset);
      if (pOre === null) {
        if (!this.warnedImpossibleCluster) {
          this.onWarning("Observed mine state is inconsistent with every ore cluster; using per-tile estimates.");
          this.warnedImpossibleCluster = true;
        }
        return this.perTileExpectedValues();
      }
      var targetCount = this.fullMineSeen ? this.knownSparkles.size : TARGETS_PER_MINE - this.observed.size;
      var counts = this.remainingCounts(targetCount);
      var result = Array(36).fill(0);
      var _iterator21 = _createForOfIteratorHelper(this.knownSparkles),
        _step21;
      try {
        for (_iterator21.s(); !(_step21 = _iterator21.n()).done;) {
          var _index9 = _step21.value;
          result[_index9] = this.nonOreAdjustedValue(counts.oreRemaining > 0 ? pOre[_index9] : 0, counts);
        }
      } catch (err) {
        _iterator21.e(err);
      } finally {
    ...
    key: "nonOreAdjustedValue",
    value: function nonOreAdjustedValue(pOre, counts) {
      var nonOreTotal = counts.goldRemaining + counts.crystalRemaining + counts.caveRemaining;
      var pNonOre = 1 - pOre;
      var pGold = nonOreTotal > 0 ? pNonOre * counts.goldRemaining / nonOreTotal : 0;
      var pCrystal = nonOreTotal > 0 ? pNonOre * counts.crystalRemaining / nonOreTotal : 0;
...
function mineState(board, opened, fullVisibility) {
  var state = Array(36).fill(".");
  for (var index = 0; index < 36; index++) {
    var row = Math.floor(index / 6);
    var position = (5 - row) * 6 + index % 6;
    if (opened.has(index)) state[position] = "o";else if (board[index] !== "e" && (fullVisibility || isLegal(index, opened))) {
      state[position] = "*";
    }
  }
  return state.join("");
}
function evaluateCalibration(lambda, strategy, visibility, values, dynamitePrice, objectDetectionPrice, boards, secondGoldChance) {
  var fullVisibility = visibility === "high";
  var effectTurns = 0;
  var totalValue = 0;
  var totalTurns = 0;
  var objectDetectionUses = 0;
  var _iterator = _createForOfIteratorHelper(boards),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      var controller = new StrategyController(strategy, visibility, lambda, values, secondGoldChance);
      controller.setDynamitePrice(dynamitePrice);
      controller.setDynamiteAvailable(36);
      var opened = new Set();
      for (var actions = 0; actions < 36; actions++) {
        ...
        var index = coordinateToIndex(decision.coordinate);
        var code = board[index];
        var resource = TYPE[code];
        var dynamite = code === "e" && controller.shouldUseDynamite();
        opened.add(index);
        if (dynamite) totalValue -= dynamitePrice;else {
          totalTurns++;
...
function calibrate(options) {
  var strategy = options.strategy,
    ...
    dynamitePrice = options.dynamitePrice,
    objectDetectionPrice = options.objectDetectionPrice,
    min = options.min,
    max = options.max,
    step = options.step,
  ...
  if (!Number.isInteger(boardCount) || boardCount <= 0 || !Number.isFinite(seed) || !Number.isFinite(secondGoldChance) || secondGoldChance < 0 || secondGoldChance > 1) {
    throw new Error("Calibration requires positive integer boards, a seed, and 0 <= P(2 gold) <= 1.");
  }
  var boards = makeCalibrationBoards(boardCount, seed, secondGoldChance);
  var coarse = [];
  for (var lambda = min; lambda <= max; lambda += step) {
    coarse.push(lambda);
  }
  if (coarse[coarse.length - 1] !== max) coarse.push(max);
  var rates = new Map();
  var rate = lambda => {
    var cached = rates.get(lambda);
    if (cached !== undefined) return cached;
    var result = evaluateCalibration(lambda, strategy, visibility, values, dynamitePrice, objectDetectionPrice, boards, secondGoldChance);
    rates.set(lambda, result.rate);
    return result.rate;
  };
  var provisionalTotal = coarse.length + fineSteps;
  var completed = 0;
  var best = coarse[0];
  for (var _i = 0, _coarse = coarse; _i < _coarse.length; _i++) {
    var _lambda = _coarse[_i];
    if (rate(_lambda) > rate(best)) best = _lambda;
    onProgress === null || onProgress === void 0 || onProgress(++completed, provisionalTotal, _lambda);
  }
  ...
  for (var point = 1; point <= fineSteps; point++) {
    ...
    onProgress === null || onProgress === void 0 || onProgress(++completed, provisionalTotal, _lambda2);
  }
  var bestRate = rate(best);
...
function calculateMiningValue(collected, used, values, costs, turns) {
  var actualMeatSpent = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 0;
  var total = (entries, prices) => {
    var value = 0;
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        value += (prices.get(item) ?? 0) * quantity;
      }
    } catch (err) {
      _iterator.e(err);
    } finally {
  ...
  var grossValue = total(collected, values);
  var expectedCost = total(used, costs);
  var netValue = grossValue - expectedCost;
  return {
    grossValue,
    expectedCost,
    actualMeatSpent,
...
function printError(message) {
  kolmafia.print(message, "red");
}
function explain(message) {
  if (!args.explain) return;
  kolmafia.printHtml("<pre color=\"green\">[EXPLAIN] ".concat(message, "</pre>"));
}
function assureHotResistance() {
  if (kolmafia.numericModifier($modifier(_templateObject$2 || (_templateObject$2 = _taggedTemplateLiteral(["Hot Resistance"])))) < 15) {
...
function mineCoordinate(coords) {
  ...
  if (!isValidCoordinate(coords)) {
    throw new Error("Invalid mining coordinate: ".concat(coords.join(",")));
  }
  return mineCoordinate$1(Mine.VOLCANO, coords);
}

function recordItemUse(accounting, item) {
...
var MiningEngine = /*#__PURE__*/function (_Engine) {
  function MiningEngine(tasks, accounting) {
    var _this;
    _classCallCheck(this, MiningEngine);
    _this = _callSuper(this, MiningEngine, [tasks]);
    _this.session = Session.current();
    _this.accounting = accounting;
    return _this;
  }
  _inherits(MiningEngine, _Engine);
  return _createClass(MiningEngine, [{
    key: "destruct",
    value: function destruct() {
      _superPropGet(MiningEngine, "destruct", this)([]);
      var diff = Session.current().diff(this.session);
      var collected = _toConsumableArray(diff.items).filter(_ref => {
        var _ref2 = _slicedToArray(_ref, 2),
          item = _ref2[0],
          quantity = _ref2[1];
        return quantity > 0 && this.accounting.values.has(item);
      });
      var expectedInputs = new Map(this.accounting.used);
      var _iterator = _createForOfIteratorHelper(diff.items),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var _step$value = _slicedToArray(_step.value, 2),
      ...
      printHighlight("oreo has run ".concat(diff.totalTurns, " turns."));
      printHighlight("Items collected:");
      var _iterator2 = _createForOfIteratorHelper(collected),
        _step2;
      try {
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          var value = (this.accounting.values.get(_item) ?? 0) * _quantity;
          printHighlight(" ".concat(_item, ": ").concat(_quantity).concat(value > 0 ? " (".concat(value, " Meat)") : ""));
        }
      } catch (err) {
        _iterator2.e(err);
      } finally {
      ...
      var _iterator3 = _createForOfIteratorHelper(this.accounting.used),
        _step3;
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          var _step3$value = _slicedToArray(_step3.value, 2),
            _item2 = _step3$value[0],
            _quantity2 = _step3$value[1];
          var cost = (this.accounting.costs.get(_item2) ?? 0) * _quantity2;
          printHighlight(" ".concat(_item2, ": ").concat(_quantity2, " (").concat(cost, " Meat)"));
        }
      } catch (err) {
        _iterator3.e(err);
      } finally {
      ...
      var summary = calculateMiningValue(collected, expectedInputs, this.accounting.values, this.accounting.costs, diff.totalTurns, this.accounting.actualMeatSpent);
      printHighlight("Gross collected value: ".concat(summary.grossValue, " Meat"));
      printHighlight("Expected item cost: ".concat(summary.expectedCost, " Meat"));
      printHighlight("Actual Meat spent on purchases: ".concat(summary.actualMeatSpent, " Meat"));
      printHighlight("Total value achieved: ".concat(summary.netValue, " Meat"));
      printHighlight(summary.valuePerAdventure !== null ? "Session value: ".concat(summary.valuePerAdventure.toFixed(1), " Meat/Adventure") : "Session value: N/A (no adventures spent)");
    }
  }, {
    key: "execute",
    value: function execute(task) {
      var meatBefore = kolmafia.myMeat();
      try {
        _superPropGet(MiningEngine, "execute", this, 3)([task]);
      } finally {
        this.accounting.actualMeatSpent += Math.max(0, meatBefore - kolmafia.myMeat());
      }
    }
  }, {
    key: "setCombat",
    value: function setCombat(task, taskCombat, taskResources) {
      // If no combats can take place here, don't bother with CCS or autoattack
      if (task.noCombat) return;
      _superPropGet(MiningEngine, "setCombat", this)([task, taskCombat, taskResources]);
    }
  }]);
}(Engine);
_defineProperty(MiningEngine, "defaultSettings", _objectSpread2(_objectSpread2({}, Engine.defaultSettings), {}, {
  logPreferenceChangeFilter: "".concat(Engine.defaultSettings.logPreferenceChangeFilter, ",mineLayout6,mineState6,lastAdventure")
}));

function detectionPotionId(getPreference) {
  for (var itemId = 819; itemId <= 827; itemId++) {
    if (getPreference("lastBangPotion".concat(itemId)) === "detection") return itemId;
  }
  return null;
}

function resolvePrice(value, item) {
  var setting = parsePrice(value);
  if (setting === null) kolmafia.abort("Invalid value \"".concat(value, "\" for ").concat(item, ". Use a number or \"mall\"."));
  if (setting !== "mall") return setting;
  var price = kolmafia.mallPrice(item);
  if (price <= 0) kolmafia.abort("Could not determine a Mall price for ".concat(item, "."));
  return price;
}
function objectDetectionPotion() {
  var itemId = detectionPotionId(kolmafia.getProperty);
  if (itemId === null) kolmafia.abort("Identify the potion of detection before using high visibility.");
  return kolmafia.toItem(itemId);
}
function resolveObjectDetectionPrice(value) {
  var setting = parsePrice(value);
  if (setting === null) {
    kolmafia.abort("Invalid objectDetectionPrice \"".concat(value, "\". Use a number or \"mall\"."));
  }
  return setting === "mall" ? resolvePrice(value, objectDetectionPotion()) : setting;
}

var MINE_RESULT_IMAGES = {
  rawvelvet: "ore",
  goldnugget: "gold",
  nacrystal1: "crystal",
  hp: "cave"
};
function parseMineLayout(layout) {
  var results = [];
  var pattern = /#(\d+)<img[^>]*\/(rawvelvet|goldnugget|nacrystal1|hp)\.gif/g;
  var match;
  while ((match = pattern.exec(layout)) !== null) {
    var which = Number(match[1]);
    var coordinate = [which % 8, Math.floor(which / 8)];
...
var dynamite = $item(_templateObject4$1 || (_templateObject4$1 = _taggedTemplateLiteral(["minin' dynamite"])));
var sheetMetal = $item(_templateObject5$1 || (_templateObject5$1 = _taggedTemplateLiteral(["heat-resistant sheet metal"])));
var brokenDrill = $item(_templateObject6$1 || (_templateObject6$1 = _taggedTemplateLiteral(["broken high-temperature mining drill"])));
var miningDrill = $item(_templateObject7$1 || (_templateObject7$1 = _taggedTemplateLiteral(["high-temperature mining drill"])));
var medicalKit = $item(_templateObject8 || (_templateObject8 = _taggedTemplateLiteral(["hippy medical kit"])));
function minedResource(before) {
  if (kolmafia.itemAmount(gold) > before.gold) return "gold";
  if (kolmafia.itemAmount(velvet) > before.ore) return "ore";
  if (kolmafia.itemAmount(crystal) > before.crystal) return "crystal";
  return null;
}
function buildMiningTasks(controller, accounting) {
  var dynamitePrice = resolvePrice(args.dynamitePrice, dynamite);
  ...
  var detectionPotion = args.visibility === "high" ? objectDetectionPotion() : null;
  if (detectionPotion) {
  ...
  var selectDecision = () => {
    ...
    controller.update(getState(Mine.VOLCANO), hasObjectDetection(Mine.VOLCANO), parseMineLayout(get("mineLayout6")));
  ...
  return [{
    ...
    do: () => {
      var drillBefore = kolmafia.itemAmount(miningDrill);
      kolmafia.use(1, brokenDrill);
      if (kolmafia.itemAmount(miningDrill) > drillBefore) {
    ...
    do: () => {
      ...
      var before = {
        ore: kolmafia.itemAmount(velvet),
        gold: kolmafia.itemAmount(gold),
        crystal: kolmafia.itemAmount(crystal)};
      var dynamiteBefore = kolmafia.itemAmount(dynamite);
      mineCoordinate(decision.coordinate);
      var dynamiteUsed = dynamiteBefore - kolmafia.itemAmount(dynamite);
...
function runCalibration(values) {
  if (args.strategy !== "ev" && args.strategy !== "ev-cluster") {
    kolmafia.abort("Calibration is only available for the ev and ev-cluster strategies.");
  }
  if (args.calibrationMin <= 0 || args.calibrationMax <= args.calibrationMin || args.calibrationStep <= 0 || args.calibrationFineSteps < 0 || !Number.isInteger(args.calibrationBoards) || args.calibrationBoards <= 0) {
    kolmafia.abort("Calibration requires 0 < min < max, step > 0, fineSteps >= 0, " + "and positive integer boards.");
  }
  ...
  var dynamitePrice = resolvePrice(args.dynamitePrice, $item(_templateObject || (_templateObject = _taggedTemplateLiteral(["minin' dynamite"]))));
  var objectDetectionPrice = args.visibility === "high" ? resolveObjectDetectionPrice(args.objectDetectionPrice) : 0;
  ...
  kolmafia.print("Use: oreo strategy=".concat(args.strategy, " visibility=").concat(args.visibility, " ") + "lambda=".concat(result.lambda, " oreValue=").concat(values.ore, " goldValue=").concat(values.gold, " ") + "crystalValue=".concat(values.crystal, " dynamitePrice=").concat(dynamitePrice, " ") + "objectDetectionPrice=".concat(objectDetectionPrice, " secondGoldChance=").concat(args.secondGoldChance), "blue");
  return result.lambda;
}
function runMining(values, lambda) {
  ...
  var quest = {
    ...
    tasks: buildMiningTasks(new StrategyController(args.strategy, args.visibility, lambda, values, args.secondGoldChance, printError), accounting)
  };
  var engine = new MiningEngine(getTasks([quest]), accounting);
  try {
    engine.run();
  } finally {
    engine.destruct();
...
function main() {
  ...
  var _argstring$trim$split = argstring.trim().split(/\s+/),
    _argstring$trim$split2 = _toArray(_argstring$trim$split),
    command = _argstring$trim$split2[0],
    rest = _arrayLikeToArray(_argstring$trim$split2).slice(1);
  var calibrationOnly = command.toLowerCase() === "calibrate";
  ...
  if (!STRATEGIES.includes(args.strategy)) {
    kolmafia.abort("Unknown strategy \"".concat(args.strategy, "\". Choose: ").concat(STRATEGIES.join(", "), "."));
  }
  if (!VISIBILITIES.includes(args.visibility)) {
    kolmafia.abort("Unknown visibility \"".concat(args.visibility, "\". Choose: ").concat(VISIBILITIES.join(", "), "."));
  }
  if (args.lambda < 0) kolmafia.abort("lambda must be zero (automatic) or positive.");
  if (!Number.isFinite(args.secondGoldChance) || args.secondGoldChance < 0 || args.secondGoldChance > 1) {
    kolmafia.abort("secondGoldChance must be between zero and one.");
  }
  if (parsePrice(args.objectDetectionPrice) === null) {
    kolmafia.abort('objectDetectionPrice must be non-negative or "mall".');
  }
  if (parsePrice(args.dynamitePrice) === null) {
    kolmafia.abort('dynamitePrice must be non-negative or "mall".');
  }
  for (var _i = 0, _arr = [["oreValue", args.oreValue], ["goldValue", args.goldValue], ["crystalValue", args.crystalValue]]; _i < _arr.length; _i++) {
    var _arr$_i = _slicedToArray(_arr[_i], 2),
      name = _arr$_i[0],
      value = _arr$_i[1];
    if (parsePrice(value) === null) kolmafia.abort("".concat(name, " must be non-negative or \"mall\"."));
  }
  var values = {
    ore: resolvePrice(args.oreValue, $item(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["unsmoothed velvet"])))),
    gold: resolvePrice(args.goldValue, $item(_templateObject6 || (_templateObject6 = _taggedTemplateLiteral(["1,970 carat gold"])))),
    crystal: resolvePrice(args.crystalValue, $item(_templateObject7 || (_templateObject7 = _taggedTemplateLiteral(["New Age healing crystal"])))),
    cave: 0
  };
  ...
  if (calibrationOnly || args.calibrate) {
    ...
    kolmafia.print("Continuing with calibrated lambda=".concat(lambda, "."), "blue");
  }
